...
async function callHunyuanVision(imageBase64, json, API_KEY, goodsName, examples) {
  ...
  if (goodsName.includes('冷藏') || goodsName.includes('低温')) {
    ...
    qtyColRule = '【你正在录入的筐类型：常温】（仅供参考，表格实际是什么类型以表头和列结构为准）\ncols 必须同时输出两个键：「整箱数量」和「物流箱」，一个都不能少！\n「整箱数量」取整箱数量列的数字\n「物流箱」取物流箱列的数字\n两列是不同数据，数值相同也要分别输出\n✅ 输出例子：{"整箱数量":10,"物流箱":5}\n如果图上没有"整箱数量"列，整箱数量=0，name后加"(未找到整箱数量列)"\n如果图上没有"物流箱"列，物流箱=0，name后加"(未找到物流箱列)"\n⚠️ 绝对禁止只输出一个键（只给整箱数量、漏掉物流箱 = 严重错误！）';
  } else {
    qtyColRule = '【你正在录入的筐类型：未知】（仅供参考，表格实际是什么类型以表头和列结构为准）\n根据表头判断单据类型后取对应列：\n若判定常温 → cols 必须同时输出「整箱数量」和「物流箱」两个键，一个都不能少（例子：{"整箱数量":10,"物流箱":5}）\n若判定冷冻 → 取「物流箱」；冷藏 → 「物流篮」；面包 → 「面包筐」\n【绝对严禁】整箱数量列仅限常温单据使用，冷冻/冷藏/面包严禁取！\n如果找不到对应列，填 0，name 后加注释说明缺哪一列';
  }
