...
async function callHunyuanVision(imageBase64, json, API_KEY, goodsName, examples) {
  ...
  const prompt = [
    ...
    '表格结构：第一行是表头（列名），下面每行是一个门店的数据。',
    '请输出一个 JSON 对象，不要输出任何其他文字，不要 markdown 代码块。',
    '格式：{"detectedType":"常温","headers":["路线编号","到店时间","门店名称","整箱数量","物流箱","纸箱","笼车","保温袋","修正单张"],"rows":[{"code":"42-1","name":"门店名","vals":[10,1,0,0,0,1]}]}',
    exampleText,
    '【铁律】',
    '1. code 只取编号数字部分（如 42-1，去掉 HR/HN 等字母前缀，去掉前导0）',
    '2. headers 是表格【表头行的完整列名列表】（第一个是路线编号，第二个是到店时间，第三个是门店名称，之后是各数量列名），列名照抄表格原样',
    '3. rows 里每行 vals 是【该行除路线编号、门店名称外的所有数字，按表头列顺序严格一一对应】（到店时间如果是时间格式如06:40就跳过不放进vals；只放数量列的数字）',
    '4. 【必须行列严格对齐】数字必须属于它所在的行和列，宁可不识别也不要错位到别的行/列',
    '5. detectedType 根据表格【表头文字】精确判断单据类型：表头含"常温配送路单"→"常温"；含"冷冻配送路单"→"冷冻"；含"冷藏配送路单"→"冷藏"；含"面包配送路单"→"面包"；无法判断时输出""。绝不能混淆冷冻和常温，它们是两种单据',
    '6. 【严禁编造】只输出图中【肉眼可见的】数据行！图上有几行就输出几行，绝不能凭空补漏/续号/猜测多出来的行',
    '7. 如果编号有缺（如 42-1 后直接 42-3），照实输出 42-1 和 42-3，中间缺的不要补',
    '8. 不要包含"合计/总计"行',
    '9. 门店名称照抄表格，不要省略，不要猜测'
  ].join('\n');
